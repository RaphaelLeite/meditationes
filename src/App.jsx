...
const storage = {
  ...
  getAll: () => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      if (k) { try { d[k] = JSON.parse(localStorage.getItem(k)); } catch { d[k] = localStorage.getItem(k); } }
    }
    return d;
  }
};
...
export default function Meditationes() {
  ...
  useEffect(() => {
    ...
    const m = storage.get('morning:' + today); if (m) setMorningEntry(m);
    const e = storage.get('evening:' + today); if (e) setEveningEntry(e);
    const s = storage.get('sealed:days'); if (s) setSealedDays(s);
    const hist = {};
    for (const k of storage.listKeys('morning:')) { const entry = storage.get(k); if (entry) hist[k.replace('morning:', '')] = { morning: entry }; }
    for (const k of storage.listKeys('evening:')) { const entry = storage.get(k); const date = k.replace('evening:', ''); if (entry) hist[date] = { ...(hist[date] || {}), evening: entry }; }
    setJournalHistory(hist);
    setLoading(false);
  }, []);

  const saveMorning = () => { storage.set('morning:' + today, morningEntry); setJournalHistory(prev => ({ ...prev, [today]: { ...(prev[today] || {}), morning: morningEntry } })); };
  const saveEvening = () => { storage.set('evening:' + today, eveningEntry); setJournalHistory(prev => ({ ...prev, [today]: { ...(prev[today] || {}), evening: eveningEntry } })); };

  const sealDay = () => {
    if (!confirmSeal) { setConfirmSeal(true); return; }
    storage.set('morning:' + today, morningEntry);
    storage.set('evening:' + today, eveningEntry);
    const newSealed = { ...sealedDays, [today]: new Date().toISOString() };
    setSealedDays(newSealed); storage.set('sealed:days', newSealed); setConfirmSeal(false);
  };
  ...
  const importData = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        Object.entries(data).forEach(function(pair) { localStorage.setItem(pair[0], typeof pair[1] === 'string' ? pair[1] : JSON.stringify(pair[1])); });
  ...
  const clearAllData = () => {
    ...
    localStorage.clear(); setConfirmClear(false); window.location.reload();
  };
